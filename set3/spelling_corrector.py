...
def viterbidecoder(sentence, lexicon, logprob_bigrams):
    ...
    for level, token in enumerate(tokens, 1):
        possible_words[(level, token)] = levenstein(token, lexicon)  # we get from 1 to 5 pairs of correct words and dst
    wpl = dict()
...
#        we now have to find the previous max and store the max word the sequence
...
def baseline(sentence, lexicon, logprob_bigrams):
    v = dict()
    tokens = [token for token in word_tokenize(sentence)]
    word, dist = levenstein(tokens[0], lexicon)[0]
